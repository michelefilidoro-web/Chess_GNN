...
def test_model():
    ...
    model.load_state_dict(torch.load("untimed_gnn.pth", map_location=device))
    model.eval()

    correct = 0
    results = [] # 1. CREIAMO LA LISTA PER SALVARE I RISULTATI
    ...
    with torch.no_grad():
        for item in val_data:
            ...
            # 2. SALVIAMO PREDIZIONE E TARGET
            results.append({
                'id': item.get('id', 'N/A'),
                'target': item['mate_in'],
                'pred': pred
    ...
    # 3. SCRIVIAMO TUTTO NEL FILE JSON
    with open('untimed_results.json', 'w') as f:
        json.dump(results, f, indent=4)
    print("Dati esportati con successo in 'untimed_results.json'!")
