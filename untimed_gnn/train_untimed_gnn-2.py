import sys
# Aggiungiamo la cartella dei prof al path
...
# IMPORTIAMO TUTTO DALLA LIBRERIA DEI PROFESSORI!
...
def prepare_dataloaders(dataset_path, batch_size=64):
    ...
    for item in raw_data:
        ...
        # IL TRUCCO DA 30 E LODE:
        # I prof usano -1 come maschera di padding per ignorare i dati (x != -1).
        # Siccome noi avevamo mappato i pezzi neri a -1, li cambiamo in 2.0
        # per non farli cancellare dalla loro rete!
        g.x[g.x[:, 1] == -1.0, 1] = 2.0

        graphs.append(g)
        # Sottraiamo 1 al target perché PyTorch vuole classi da 0 a 4, non da 1 a 5
        labels.append(item['mate_in'] - 1)

    # Convertiamo le label in un tensore
    labels = torch.tensor(labels, dtype=torch.long)

    # Split rigoroso 80% / 10% / 10%
    total = len(graphs)
...
def main():
    ...
    train_loader, val_loader, test_loader = prepare_dataloaders("gnn_untimed_dataset.pt", BATCH_SIZE)
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), "untimed_gnn.pth")
    ...
    model.load_state_dict(torch.load("untimed_gnn.pth"))
    test_loss, test_acc = evaluate(model, test_loader, criterion, device)
    print(f"Accuratezza finale su dati mai visti: {test_acc:.4f}")
    print("Addestramento completato! Modello salvato come 'untimed_gnn.pth'")
